...
class MenuSelect(Screen,ConfigListScreen):
	...
	def personalizebouquet(self,result):
		B='/etc/enigma2/bouquets.tv';A='/etc/enigma2/SelectFolder'
		if result is _C:
			try:
				...
				for file in glob.glob('/etc/enigma2/userbouquet.jmx*.tv'):os.system('cp /etc/enigma2/subbouquet.jmx*.tv'+' '+A)
				for file in glob.glob('/etc/enigma2/userbouquet.iptvdiv.tv'):os.system('cp /etc/enigma2/subbouquet.suls_iptv*.tv'+' /etc/enigma2/SelectFolder')
				os.system('rm -r /etc/enigma2/*.tv');Writebouquets=open(B,'w');Writebouquets.write('#NAME User - bouquets (Tv)\n');Writebouquets.close()
				for x in jjw:
					try:
						jx=x.split(_E)
						with open(B,'a')as f:f.write('#SERVICE 1:7:1:0:0:0:0:0:0:0:FROM BOUQUET "'+jx[0].strip()+'" ORDER BY bouquet\n')
					except:pass
