...
class TerrestrialScanGio(Screen):
	...
	def __init__(A,session,args=0):
		...
		if A.uhf_vhf==_F:
			C=7
			for D in range(5,13):
				for F in H:A.scanTransponders.append({_B:channel2freq(D,C),_C:F,_D:C})
		...
		if A.uhf_vhf==_N:
			C=7;Q=177500000
			for D in list(range(0,8))+list(range(50,74)):R=Q+(D*C*1000000+(2000000 if D>8 else 0));A.scanTransponders.append({_B:R,_C:eDVBFrontendParametersTerrestrial.System_DVB_T,_D:C})
		if A.uhf_vhf=='xml':
			for E in nimmanager.getTranspondersTerrestrial(A.region):
				if E[10]<1:A.scanTransponders.append({_B:E[1],_C:eDVBFrontendParametersTerrestrial.System_DVB_T,_D:E[2]//1000000})
				if E[10]!=0:A.scanTransponders.append({_B:E[1],_C:eDVBFrontendParametersTerrestrial.System_DVB_T2,_D:E[2]//1000000})
